...
class KTOTrainer(Trainer):
    ...
    def __init__(
        ...
        with PartialState().local_main_process_first():
            ...
            if self.calculate_KL:
                ...
                print("kto_trainer | Line 677 | train_kl_dataset : ", train_kl_dataset)
                ...
                print("kto_trainer | Line 687 | train_kl_dataset after : ", train_kl_dataset)

                # merge the datasets
                train_dataset = concatenate_datasets([train_dataset, train_kl_dataset], axis=1)
                
                print("kto_trainer | Line 692 | train_dataset after : ", train_dataset)
        ...
        with torch.no_grad():
            if self.ref_model is None:
                ...
                if self.is_encoder_decoder:
                    ...
                    print("kto_trainer | Line 972 | REF MODEL IS NOT NONE")
                    completion_logits = self.ref_model(
    ...
    def get_batch_logps(
        ...
        print("Line 1021 | kto_trainer | logits.shape and labels.shape : ", logits.shape, labels.shape)
    ...
    def forward(
        ...
        model_kwargs = (
            ...
            if self.is_encoder_decoder
                ...
                "labels": batch["completion_labels"],
                "images": batch["images"],
                "dpo_forward": True                 # HOPING THIS WORKS /\
            }
        )
        if self.aux_loss_enabled:
            model_kwargs["output_router_logits"] = True
            
        # batch["images"] -> [0] -> [list[list]] (len = batch_size), [1] -> ['video', 'video', 'video', 'video'] (len = batch_size)
        # each feature is in the form of a heavily nested list in the following shape:
        # print(len(batch["images"]))                 # 2
        # print(len(batch["images"][0]))              # 4         batch_size (B)
        # print(len(batch["images"][0][0]))           # 3         n_channels (C)
        # print(len(batch["images"][0][0][0]))        # 8         # num_frames (N)
        # print(len(batch["images"][0][0][0][0]))     # 224        # H 
        # print(len(batch["images"][0][0][0][0][0]))  # 224       # W
        
        # # convert nested lists to tensors
        feature_tensors = []
        for feature in batch["images"][0]:
            feature_tensor = torch.tensor(feature, dtype=torch.bfloat16)
            # print(feature_tensor.shape)
            feature_tensors.append(feature_tensor)
        
        batch["images"][0] = feature_tensors
        
        if not self.is_encoder_decoder:
            model_kwargs["images"] = batch["images"]
        
        # print("Line 1109 | model_kwargs['images'] : ", model_kwargs["images"][1], len(model_kwargs["images"][0]), type(model_kwargs["images"][0][0]), model_kwargs["images"][0][0].shape)
        # print("kto_trainer | Line 1116 | dpo_forward : ", model_kwargs["dpo_forward"])

        completion_logits, new_labels = model(              # Instead of outputs (SPECIFIC FIX FOR LLAVA ARCHITECTURE)
            batch["completion_input_ids"],
            attention_mask=batch["completion_attention_mask"],
            **model_kwargs,
        )
        # completion_logits = outputs.logits
        # print("kto_trainer | Line 1124 | completion_logits | new_labels ", completion_logits.shape, new_labels.shape)       # Shape is the same now (4, 2165, 32000) (4, 2165)

        completion_logps = self.get_batch_logps(
            completion_logits,
            new_labels,                 # INSTEAD OF batch["completion_labels"] as that is of original shape
            average_log_prob=False,
            is_encoder_decoder=self.is_encoder_decoder,
            label_pad_token_id=self.label_pad_token_id,
        )
        
        # print("kto_trainer | Line 1134 | completion_logps : ", completion_logps.shape, completion_logps)    # (4,), List of logits
        # print("kto_trainer | Line 1135 | batch : ", batch.keys())   # Now has 'label' key
        # print("kto_trainer | Line 1136 | batch['completion_labels'] : ", batch["completion_labels"])    # Embedding for the labels ig, list of ids
    ...
    def get_batch_loss_metrics(
        ...
        print("kto_trainer | Line 1203 | Printing batch ...")
        print(batch.keys())     # dict_keys(['completion_input_ids', 'completion_labels', 'completion_attention_mask', 'images'])
        print("kto_trainer | Line 1205 | train_dataset again : ", self.train_dataset)
        # print(batch['completion_input_ids'].shape)  # torch.Size([4,112])
        # print(batch['completion_input_ids'])
        # print(batch['completion_labels'].shape)  # torch.Size([4,112])
        # print(batch['completion_labels'])
        # print(batch['completion_attention_mask'].shape)  # torch.Size([4,112])
        # print(len(batch['images'][0][0]))  # torch.Size([8, 3, 224, 224])
    ...
    def compute_loss(
        ...
        # print("Line 1297 | kto_trainer | inputs : ", inputs)
